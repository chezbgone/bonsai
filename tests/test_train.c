...
void init_valuations(CTable* ct, ValGrid const* input, LambList* ll, LambExpr* const* lib)
{
    clock_t start, end;
    float diff;
    ...
    start = clock();
    {
        ...
        for ( int pi = 0; pi != ll->len; ++pi ) {
            /* TODO: syllabize  ((ll->arr[pi].e))   
             */
            //LambExpr* syl = syllabize(ll->arr[pi].e) 
            ValGrid const* v = evaluate(input, syl, ct, nb_args);
        }
    }
    end = clock();
    diff = ((float)(end-start))/CLOCKS_PER_SEC;
